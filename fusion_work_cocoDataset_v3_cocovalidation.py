...
def preprocess_labels(labels, num_predictions=49):
    ...
    # Initialize an array with 0 (background class) for labels that don't match the number of predictions
    preprocessed_labels = np.zeros((len(labels), num_predictions), dtype=np.int32)
    
    for i, label in enumerate(labels):
        # Clip label values to the range [0, 90] to avoid out-of-range errors
...
class SSDLiteHead(layers.Layer):
    def __init__(self, num_classes=80):  # COCO has 80 classes
        ...
        # Prediction layers for class and bounding box
...
if __name__ == "__main__":
    # Load the training COCO dataset
    ...
    VAL_ANNOTATIONS_PATH = '/home/aimslab/Documents/cocoDataset/coco/annotations/instances_val2017.json'
    VAL_IMAGES_PATH = '/home/aimslab/Documents/cocoDataset/coco/images/val2017/'

    # Load the validation COCO dataset
    val_images, val_labels, val_bboxes = load_coco_dataset('/home/aimslab/Documents/cocoDataset/coco/annotations/instances_val2017.json', '/home/aimslab/Documents/cocoDataset/coco/images/val2017/')

    # Initialize the model
    model = MobileDetFusionModel(num_classes=91)  
    ...
    evaluation_results = model.evaluate(
        val_images,
        {'class_output': val_labels, 'box_output': val_bboxes},
        batch_size=32
    )

    total_loss, class_loss, box_loss, class_acc = evaluation_results

    print(f"Total Loss: {total_loss}")
    print(f"Class Output Loss: {class_loss}")
    print(f"Box Output Loss: {box_loss}")
    print(f"Class Output Accuracy: {class_acc}")
