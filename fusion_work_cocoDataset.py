...
MAX_KEYPOINTS = 500  # Adjust this based on your needs
...
def load_coco_dataset(coco_annotations_path, coco_images_path, num_samples=1000):
    ...
    for img_id in img_ids:
        ...
        image_labels = []
        for ann in anns:
            # COCO bounding boxes are in [x, y, width, height]
            bbox = ann['bbox']
            bbox = [bbox[0], bbox[1], bbox[2], bbox[3]]  # (x, y, width, height)
            image_bboxes.append(bbox)
            image_labels.append(ann['category_id'])  # COCO category ID
    ...
    labels = np.array([np.pad(label, (0, 49 - len(label)), constant_values=-1)[:49] for label in labels])  
...
# Define the full MobileDet model with the SSD Lite detection head
...
if __name__ == "__main__":
    ...
    model = MobileDetFusionModel(num_classes=80)

    # Reshape labels to match the SSD Lite output format
    labels = np.array([np.repeat(label, 49) for label in labels])
